...
- [x] Encrypt story text, scene scripts, and media at rest. *(KC-5: `storage/crypto.py`, AES-256-GCM keyed by `KC_STORAGE_KEY`. In transit: pending a network boundary. KC-10: each story now has its own data key stored wrapped by the master — crypto-shred on delete, incremental master-key rotation.)*
