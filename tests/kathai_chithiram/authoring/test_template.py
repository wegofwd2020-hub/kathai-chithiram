...
def test_shipped_example_templates_lower_to_valid_scripts():
    ...
    # Story examples only; the progress_policy.* files are ProgressPolicy JSON (a
    # different kind), not story templates.
    files = [f for f in sorted(examples.glob("*.json")) if not f.name.startswith("progress_policy")]
