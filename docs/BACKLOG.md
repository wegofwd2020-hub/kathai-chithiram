...
- **M1 · Per-child progress quantification → therapist-suggested premise customization** — quantify each child's feedback (part c) into a progress measure (e.g. % independent over last K sessions, mastery flag, regulation trend) that may *suggest* — never auto-author — an updated premise (part a) for future stories. The therapist decides; the safety gate still runs (ADR-001 Decision 5). **ADR: `docs/ADR_002_progress_quantification_premise_suggestion.md` (Proposed)** — capture-first, suggestion-only, engine deferred behind preconditions. *Keep the door open now by preserving quantifiable feedback primitives (`prompt_level` refused/prompted/independent, `completed`, `mood_checkin`) keyed to a structured `goal` id — build no engine yet.*
