...
### UC-10 — "What are we entitled to?" (the entitlement-discovery entry point)
- **Persona:** P3 lifespan navigator / parent at a first-diagnosis moment — a family that senses help must exist but doesn't know what to ask for. (Senior variant: an adult child after a parent's dementia diagnosis.)
- **Query (as asked):** "My daughter was just diagnosed with autism — what help are we entitled to that we might not even know about?"
- **Domain(s):** entitlement / benefits navigation (cross-cutting) + special-needs (variant: aging). Directly exercises **ADR-009 D10**.
- **Immediate need:** a *map* of what potentially applies, each with its **stated criteria, source, and who decides** — federal (IDEA Part C early intervention 0–3, then Part B special education; SSI; Medicaid / EPSDT; Medicaid HCBS waivers; ABLE accounts; tax provisions) and state (Michigan developmental-disability services, waivers, respite) — so a family stops discovering these by accident, years late.
- **Dependencies / what comes next:** this is the **entry point that spawns the domain-specific use-cases** — early intervention now, special education at 3, waiver applications filed early because of waitlists (UC-4), SSI and its adult reassessment at 18 (UC-4), Medicaid/EPSDT for therapies, an ABLE account to save without losing means-tested benefits, tax credits; and a standing re-check because entitlements change at 3, 18 and 22 (the lifespan axis). *The dependency society misses is the whole point here: no single office hands a family the map — each benefit has its own door, criteria and deadline, and the diagnosis moment is exactly when nobody has time to go find them.*
- **Answer layers:** education (what each program is and its stated criteria, cited) · navigation (the applications, agencies, deadlines/waitlists, who decides, and a **benefits counsellor / Family-to-Family Health Information Center / Protection & Advocacy** for the person-specific determination) · products: none.
- **Boundary flags:** **the canonical navigate-not-determine case, and the highest determination-pressure one in the log.** Answer is "here is what *exists* and its criteria; an agency determines *your* eligibility" — never "you qualify for X" (ADR-009 D3 + D10; the EU-AI-Act eligibility line). The "so do I qualify?" pull is strongest here, so the **designed handoff** to a benefits counsellor is essential, not optional. **Freshness** is safety-critical (amounts, income limits, waitlists — dated, "verify with the agency", ADR-011) and answers are **state-specific** (Michigan first, ADR-009 D4). The query names a real person ("my daughter") → surface the general entitlement map, but **do not adjudicate or store the individual** (ADR-009 D6).
- **Sources needed:** SSA (SSI childhood disability), CMS (Medicaid / EPSDT / HCBS waivers), ED / IDEA (Part C and Part B), IRS (ABLE, tax), Michigan DHHS / developmental-disability agency, Family-to-Family Health Information Centers, the state Protection & Advocacy system (federal public-domain + state; state freshness-sensitive — ADR-011).
- **Status:** captured — the entry-point use-case for Decision 10; it exercises entitlement discovery directly and is the sharpest test of the "never determine" handoff.

### UC-11 — Early intervention for a young child (IDEA Part C, ages 0–3)
- **Persona:** P3 lifespan navigator / parent of an infant or toddler not meeting milestones — the earliest entry point, often before any diagnosis.
- **Query (as asked):** "My 18-month-old isn't talking or walking yet — is there help before he starts school?"
- **Domain(s):** early intervention (IDEA Part C) + special-needs practice + entitlement navigation (a specific unfolding of UC-10)
- **Immediate need:** understand Early Intervention under IDEA Part C — a federal entitlement for children 0–3 with developmental delays or disabilities — and the three things families are rarely told: **you can self-refer** (no diagnosis or doctor's referral required), the **evaluation is free**, and it is **time-sensitive** (an evaluation timeline of ~45 days, and earlier support means better outcomes). The plan it produces is the **IFSP** (Individualized Family Service Plan).
- **Dependencies / what comes next:** the referral → free developmental evaluation → an eligibility decision by the state program (navigate, don't determine); the IFSP and its services (speech, OT, PT, developmental therapy — home- and community-based, and family-centred, so **parents are coached, not sidelined**); cost (evaluation and IFSP development are largely free under Part C; some services use a sliding scale / insurance / Medicaid EPSDT); the **transition at age 3 to Part B special education / preschool** — a cliff whose planning starts *before* the third birthday (bookends UC-4's 18–22 transition); re-evaluation over time; and, if found ineligible, the fallbacks (developmental monitoring, private therapy, Medicaid/EPSDT). *The dependency society misses: the barrier is usually **awareness, not eligibility** — families wait for a doctor to "send" them when they could refer today, and the clock is running.*
- **Answer layers:** education (what Part C is, self-referral, the IFSP, developmental milestones, cited) · navigation (the state EI program and local referral point, the evaluation, who provides services, the Part C→Part B transition, who decides) · products: none.
- **Boundary flags:** navigate-not-determine (the state EI program decides eligibility after evaluation — surface the criteria and how to refer, never "your child qualifies"); a developmental concern is **educational, not diagnostic** ("these are the typical milestones; a developmental evaluation assesses them" — never "your child has autism", ADR-009 D3 / CONTENT_SAFETY §3). Part C is federal but **state-administered** (Michigan's programme is *Early On*) — state-specific (ADR-009 D4), freshness-sensitive (contacts/process — ADR-011). The query names a real child → surface the general service map and how to refer, don't adjudicate the individual (ADR-009 D6).
- **Sources needed:** ED / IDEA Part C statute and regulations (federal, public domain), CDC "Learn the Signs. Act Early." milestones, Michigan's Early On programme, CMS / Medicaid EPSDT, the ECTA Center (state/local details freshness-sensitive — ADR-011).
- **Status:** captured — the 0–3 bookend to the lifespan; its lesson is that the barrier is awareness and timing, not qualifying.

### UC-12 — Special education and the IEP (IDEA Part B, ages 3–21)
- **Persona:** P2 helper-learner / P3 lifespan navigator — a parent of a school-age child with a disability (an older student is a self-advocate). The central school-years domain, bridging UC-11 (0–3) and UC-4 (18–22).
- **Query (as asked):** "My son has a learning disability — how do I get the school to actually support him?"
- **Domain(s):** special education (IDEA Part B) + disability rights + entitlement navigation
- **Immediate need:** understand special education under IDEA Part B — the right to a **Free Appropriate Public Education (FAPE)** in the **Least Restrictive Environment (LRE)**; the process (a **written** evaluation request → evaluation → eligibility → the **IEP**); what an IEP contains (goals, services, accommodations, placement); that a **parent is an equal member of the IEP team**; and the **504 plan vs IEP** distinction (a 504 plan = accommodations under Section 504/ADA; an IEP = specialised instruction under IDEA).
- **Dependencies / what comes next:** the eligibility decision across IDEA's categories (navigate, don't determine); developing goals/services/placement at the IEP meeting; the parent's **procedural rights** — prior written notice, consent, access to records, an IEP reviewed at least annually, re-evaluation every three years; **dispute resolution when you disagree** — IEP facilitation, mediation, a state complaint, a due-process hearing (the leverage families most often don't know they have); an **independent educational evaluation** at public expense; discipline protections (manifestation determination); extended school year; assistive technology written into the IEP (links UC-6); and the **transition IEP that starts at 14–16** (links UC-4). *The dependency society misses: the IEP is a legally enforceable document with procedural rights and a dispute ladder — parents have real leverage as equal team members, but are often treated as guests and never told.*
- **Answer layers:** education (FAPE/LRE/IEP/504, the process, the procedural rights, cited) · navigation (how to make the written evaluation request, the timelines, the dispute-resolution options, the Parent Training and Information center, an advocate, and — for due process — an education attorney) · products: none.
- **Boundary flags:** **navigate-not-determine** — whether a child fits an eligibility category, and whether a given service is "appropriate"/FAPE, is decided by the IEP team or a hearing officer; surface the standards and the process, never "the school must provide X" or "your child qualifies under Y". Dispute strategy is **legal-information-not-advice** (point to a PTI / advocate / attorney, don't advise "file for due process"). IDEA is federal but **state-administered** — complaint processes and timelines vary (Michigan; ADR-009 D4), freshness-sensitive (ADR-011). The query names a real child → surface the general map and process, don't adjudicate the case (ADR-009 D6).
- **Sources needed:** ED / IDEA Part B statute and regulations, OSEP guidance, Section 504 (ED Office for Civil Rights) — all federal public domain; CADRE (dispute resolution), the state Parent Training and Information center and state complaint procedures (state-level, freshness-sensitive — ADR-011).
- **Status:** captured — the central school-years case; its lesson is that the IEP is enforceable and parents hold procedural leverage they're rarely told about.

### UC-13 — Making supported decision-making work (the affirmative complement to UC-9)
- **Persona:** P3 lifespan navigator / parent committed to preserving autonomy, and the **self-advocate** (the adult who wants to make their own choices with support). Where UC-9 enters at "do we need guardianship?", this enters at "how do we support autonomy?" — the same spectrum from the other end.
- **Query (as asked):** "I don't want to take over my adult daughter's life — how do we set up support so she can make her own decisions?"
- **Domain(s):** legal / decision-making rights + autonomy + special-needs (complements UC-9)
- **Immediate need:** understand **supported decision-making (SDM)** as the affirmative model — the person keeps their legal rights and chooses trusted supporters to help them understand, weigh and communicate decisions — plus the toolkit that operationalises it: a **Supported Decision-Making Agreement** (recognised by statute in a growing number of states), durable and healthcare **powers of attorney**, a **healthcare proxy / advance directive**, a **representative payee** for benefits, **release-of-information / HIPAA authorisations** so supporters can actually access information, and an **ABLE account** for financial autonomy. The governing idea is the **dignity of risk** — the right to make one's own choices, including imperfect ones.
- **Dependencies / what comes next:** the hard part is **getting institutions to honour it** — banks, doctors, schools and benefits agencies often default to "who's the guardian?", so the documents plus the confidence to push back matter as much as the agreement itself; **state recognition** (does the state have an SDM-agreement statute? Michigan's status — navigate); building decision-making skill over time (it grows with practice); **layering tools** (SDM *and* POA *and* ABLE, not either/or); crisis/emergency coverage (does a supporter have authority? the healthcare proxy fills the gap); revisiting as needs change; and the honest fallback — a **limited** guardianship as a genuine last resort (cross-ref UC-9). *The dependency society misses: setting SDM up on paper is easy; the friction is operational — making a bank teller or an ER accept it — and no one warns families of that.*
- **Answer layers:** education (the SDM model, the toolkit, dignity of risk, and that least-restrictive is the legal preference, cited) · navigation (SDM-agreement templates and state recognition, the state Protection & Advocacy org, an attorney for the documents, self-advocacy organisations) · products: none.
- **Boundary flags:** that **least-restrictive support is the legal preference is a fact and may be stated**; *which* tools a particular family should adopt, and drafting the documents, are **legal-information-not-advice** → point to an attorney / P&A, never "use SDM instead of guardianship" as an individualised recommendation. **State-specific** (SDM-agreement recognition varies — Michigan, ADR-009 D4), freshness-sensitive (ADR-011). Describes a relationship, not a capacity adjudication of a specific person (ADR-009 D6). Deeply aligned with the **mission** and ADR-001 — presume competence.
- **Sources needed:** the National Resource Center for Supported Decision-Making, state SDM-agreement statutes (Michigan), ACL, ABA, the state Protection & Advocacy system, the ABLE National Resource Center, self-advocacy organisations (state-level details freshness-sensitive — ADR-011).
- **Status:** captured — the affirmative, operational complement to UC-9; its lesson is that autonomy-preserving support is a toolkit whose real difficulty is getting institutions to honour it.
